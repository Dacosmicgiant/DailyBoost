...
async function getTodosCollection(): Promise<Collection> {
  ...
  try {
    ...
    if (client) {
        try { await client.close(); } catch (closeErr) { /* ignore */ }
    }
...
// Keep the rest of your handlers as they were in the previous refactor
// (using `await getTodosCollection()` and NO `client.close()` in finally)
...
// ... (PUT and DELETE handlers similarly updated to use getTodosCollection) ...
